...
const FRAME_PATH = '/coffee-frames';  // Folder containing frame_0.webp to frame_119.webp
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      try {
        ...
        // Immediately trigger initial render if images exist
        ...
        // Even if some fail, we can try to proceed with loaded ones if needed, 
        // but for now we'll set loaded to true so it doesn't block forever if images are missing.
        setImagesLoaded(true);
      }
    };
  ...
  useEffect(() => {
    ...
    const renderFrame = () => {
      ...
      if (img.complete) {
        ...
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        // Calculate scaling (contain fit, centered)
        const scale =
          FIT_MODE === 'contain'
            ? Math.min(canvas.width / img.width, canvas.height / img.height)
            : Math.max(canvas.width / img.width, canvas.height / img.height);
    ...
    return (
      <div className="fixed inset-0 bg-[#1A0F0A] flex flex-col items-center justify-center z-50">
        ...
        <p className="text-amber-100/70 text-lg font-['Inter']">
          Loading your coffee moment... {Math.round(loadProgress)}%
        </p>
      </div>
    );
  }
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-amber-100/70 font-['Inter'] drop-shadow-md">
              From bean to cup, we keep quality in every sip
            </p>
          </motion.div>
