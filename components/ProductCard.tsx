...
export default function ProductCard({ product, index }: ProductCardProps) {
  ...
  const cardImageAlt: Record<string, string> = {
    Cappuccino: 'Cappuccino coffee image',
    Latte: 'Latte coffee image',
    Mocha: 'Mocha coffee image',
  };

  const imageSrc = cardImageSrc[product.name] ?? product.image;
  const imageAlt = cardImageAlt[product.name] ?? `${product.name || 'Coffee'} menu image`;
